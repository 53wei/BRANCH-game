...
export class BorrowAnchorController {
  ...
  restore(state: Pick<MechanicSaveState, "borrowedObject" | "anchorSlot">): void {
    this.reset();
    if (!state.borrowedObject || !this.configs.has(state.borrowedObject.borrowedObjectId)) return;
    this.borrowed = { ...state.borrowedObject };
    this.slot = state.borrowedObject.anchored && state.anchorSlot.borrowedObjectId
      ? { ...state.anchorSlot }
      : { borrowedObjectId: null };
    this.bindings.createBorrowedObject(this.borrowed);
    this.bindings.setBorrowedObjectAnchored(this.borrowed.borrowedObjectId, this.borrowed.anchored);
  }
