...
describe("campaign manifest", () => {
  ...
  it("marks the first four chapters as playable", () => {
    expect(campaignManifest.chapters.filter((chapter) => chapter.status === "playable").map((chapter) => chapter.id)).toEqual(["west-corridor-loop", "north-tower-ledger", "front-hall-guest", "sealed-pavilion"]);
  });
