...
export const PROLOGUE_EVIDENCE: readonly SliceEvidenceDefinition[] = [
  {
    ...
    label: "查看七年前的离家记录",
    title: "离家记录",
    body: "记录写着：赵映于傍晚六点十分离开听雨轩。纸角有非常浅的压痕，像原本在同一位置写过另一组数字又被处理掉。",
    note: "现在还看不清原来写了什么，也不能只凭这道压痕断定是谁动过记录。",
...
export const CH1_TRACE_SEARCH_AREA = {
  id: "west-trace-search-area",
  label: "墙脚痕迹搜索范围",
  position: [
    CH1_TRACES.reduce((sum, item) => sum + item.position[0], 0) / CH1_TRACES.length,
    0.9,
    CH1_TRACES.reduce((sum, item) => sum + item.position[2], 0) / CH1_TRACES.length,
  ] as Vec3Tuple,
  radius: 2.4,
} as const;
